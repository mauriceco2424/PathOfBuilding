...
-- Batch version of calc_with: tests multiple candidates in a single IPC round-trip.
-- Eliminates per-call overhead (JSON serialize/parse, pipe I/O, redundant baseOutput copies).
-- params: { candidates: { addNodes?: number[], removeNodes?: number[], masteryOverrides?: table, conditions?: string[] }[], useFullDPS?: boolean }
-- Returns: { results = [{output = ...}, ...], baseOutput = ... } or nil, error
function M.calc_with_batch(params)
  if not build or not build.calcsTab then return nil, 'build not initialized' end
  local calcFunc, baseOut = build.calcsTab:GetMiscCalculator()
  local useFullDPS = params and params.useFullDPS
  local candidates = (params and params.candidates) or {}
  local safeBaseOut = deepCopySafe(baseOut)
  local results = {}

  for i, cand in ipairs(candidates) do
    local override = {}
    if cand and type(cand.addNodes) == 'table' then
      local addNodes = {}
      local hasAddNodes = false
      for _, id in ipairs(cand.addNodes) do
        local n = build.spec and build.spec.nodes and build.spec.nodes[tonumber(id)]
        if n then addNodes[n] = true; hasAddNodes = true end
      end
      if hasAddNodes then override.addNodes = addNodes end
    end
    if cand and type(cand.removeNodes) == 'table' then
      local removeNodes = {}
      local hasRemoveNodes = false
      for _, id in ipairs(cand.removeNodes) do
        local n = build.spec and build.spec.nodes and build.spec.nodes[tonumber(id)]
        if n then removeNodes[n] = true; hasRemoveNodes = true end
      end
      if hasRemoveNodes then override.removeNodes = removeNodes end
    end
    if cand and type(cand.masteryOverrides) == 'table' then
      override.masteryOverrides = {}
      for nodeIdStr, effectId in pairs(cand.masteryOverrides) do
        local nid = tonumber(nodeIdStr)
        local eid = tonumber(effectId)
        if nid and eid then
          override.masteryOverrides[nid] = eid
    ...
    if cand and type(cand.conditions) == 'table' then
      override.conditions = cand.conditions
    end
    local out = calcFunc(override, useFullDPS)
    results[i] = { output = deepCopySafe(out) }
  end

  return { results = results, baseOutput = safeBaseOut }
end
